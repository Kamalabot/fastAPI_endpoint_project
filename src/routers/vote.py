...
from .. import comm_schema, databaseORM, models, oauth
...
def vote(vote: comm_schema.Vote, db: Session = Depends(databaseORM.get_db), 
    ...
    if (vote.dir == 1):
        if found_vote:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT,
                                detail=f"user {current_user} has alredy voted on post {vote.post_id}")
